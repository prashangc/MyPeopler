...
import 'iHttpService.dart';

@Injectable(as: IHttpService, env: [Env.dev])
class DevHttpService implements IHttpService {
  late Dio _dio;

  DevHttpService() {
    _dio = Dio();
    _dio
      ..options.baseUrl = DEV_BASEURL
  ...
  Future<Response> post(
    ...
    try {
      ...
      if (isDebug) {
        log(response.toString(), name: "HttpService Response: ");
      }
      return response;
    } on DioException catch (e) {
      if (e.response != null) {
  ...
  initializeInterceptors() {
    _dio.interceptors.add(
      InterceptorsWrapper(onError: (error, handler) {
        log(error.toString(), name: "HttpService");
